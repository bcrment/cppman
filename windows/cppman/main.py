...
# All Rights reserved.
# Adapted by Bruno C. Rodrigues <bcrment@gmail.com>
#
...
def sanitize_filename(filename):
    return re.sub(r'\s+', '_', re.sub(r'[^A-Za-z0-9_\-]', '_', filename))
...
class Cppman(Crawler):
    ...
    def man(self, pattern):
        ...
        page_filename = sanitize_filename(self.get_normalized_page_name(page_name))
        ...
        gz_path = self.get_page_path(environ.source, page_name)
        
        cmd = [
            environ.pager_script,
            pager_type,
            gz_path,
            str(columns),
        ...
        cmd_str = ' '.join(cmd)
        ## print("CMD running:", cmd_str)

        pid = subprocess.Popen(cmd_str, shell=True)
        pid.wait()
    ...
    def get_page_path(self, source, name):
        safe_name = sanitize_filename(self.get_normalized_page_name(name))
        return os.path.join(environ.cache_dir, source, safe_name + '.3.gz')
